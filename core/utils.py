...
def draw_bbox_gt(image2, list_bb, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    """
    bboxes: [x_min, y_min, x_max, y_max, probability, cls_id] format coordinates.
    """

    num_classes = len(classes)
    image_h, image_w, _ = image2.shape
    hsv_tuples = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]
    colors = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuples))
    colors = list(map(lambda x: (int(x[0] * 255), int(x[1] * 255), int(x[2] * 255)), colors))
    ...
    for r in list_bb:
        elem = [float(f) for f in r.split(",")]
        x = int(elem[0])
        y = int(elem[1])
        w = int((elem[2] - elem[0]))
        h = int((elem[3] - elem[1]))
        color = (0, 255, 0)
        cv2.rectangle(image2, (x, y), (x + w, y + h), color, 8)
        # cv2.imshow('Features', im)
        # vect = main_class[int(elem[4])]
        #int(elem[4])
        cv2.putText(image2, '%s' % classes[int(elem[4])], (x + 5, 30 + h + y),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.5, color,
                    thickness=5)
