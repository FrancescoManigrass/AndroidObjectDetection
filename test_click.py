...
from core.utils import get_center, IsContained
...
for thresh1 in thresholds:
    ...
    args.output = join(first_path, thresh1.__str__())
    print(args.output)
            ...
            try:
                if "difficult" in line:
                    ...
                    if one_class:
                        class_name = "noclass"
                    """
                    cv2.rectangle(board, (int(left), int(top), int(right) - int(left), int(bottom) - int(top)),
                                  (255, 0, 0), 2)
                    """
  ...
  """
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for txt_file in predicted_files_list:
            ...
            # cv2.imwrite("mAP/predict_images/" + os.path.basename(txt_file).replace(".txt", ".jpg"), board)
  ...
  """
    ...
    ap_dictionary = {}
    performance_dict = {}
    total_tp = [0 for c in range(n_classes)]
    total_fp = [0 for c in range(n_classes)]
    total_gt = [0 for c in range(n_classes)]
    ...
    with open(results_files_path + "/results.txt", 'w') as results_file:
      ...
      """
            ...
            for idx, prediction in enumerate(predictions_data):
                ...
                if not overlapping_metric:
                    ...
                    for obj in ground_truth_data:
                        ...
                        if obj["class_name"] == class_name:
                            bbgt = [float(x) for x in obj["bbox"].split()]
                            # bb predicted bb
                            bb_max = [min(bb[0], bbgt[0]), min(bb[1], bbgt[1]), max(bb[2], bbgt[2]),
                                      max(bb[3], bbgt[3])]
                            bb_min = [max(bb[0], bbgt[0]), max(bb[1], bbgt[1]), min(bb[2], bbgt[2]),
                                      min(bb[3], bbgt[3])]
                            if bb_max[0] <= bb_min[0] <= bb_max[2] and bb_max[0] <= bb_min[2] <= bb_max[2]:
                                if bb_max[1] <= bb_min[1] <= bb_max[3] and bb_max[1] <= bb_min[3] <= bb_max[3]:
                                    # check that the average point of bbmax is inside min box
                                    if bb_min[1] <= bb_max[1] + ((bb_max[3] - bb_max[1]) / 2) <= bb_min[3]:
                                        ovmax = 1
                                        gt_match = obj
                ...
                if ovmax >= min_overlap:
                    if "difficult" not in gt_match:
                        if not bool(gt_match["used"]):
                            ...
                            fp[idx] = 1
                            fp_bb.append(predictions_data[idx])
                            if show_animation:
                ...
                """
                #gt_match contained the ground truth matched
                #prediction contain the prediction
                centerOfPrediction=get_center(prediction["bbox"])
                if IsContained(gt_match["bbox"],centerOfPrediction):
                    #TP
                    tp[idx]=1
                    tp_bb.append(predictions_data[idx])
                    gt_match["file_id"] = file_id
        ...
        """
            ...
            for pred in [os.path.basename(f).replace(".txt", "") for f in ground_truth_files_list]:
                print("dfd")
                path_image_performance = join(os.path.curdir, args.output, "mAP",
                                              "PredictedImagePerformance" + "_" + IntersectionMethod)
                ...
                for bb1 in [tp2['bbox'] for tp2 in tp_bb if tp2['file_id'] == pred]:
                    ...
                    # print(bb1.__str__())
                    cv2.rectangle(img_performance, (int(bb2[0]), int(bb2[1])), (int(bb2[2]), int(bb2[3])), colors['tp'],
                                  2)
                for bb1 in [tp2['bbox'] for tp2 in fp_bb if tp2['file_id'] == pred]:
                    bb2 = [float(x) for x in bb1.split()]
                    cv2.rectangle(img_performance, (int(bb2[0]), int(bb2[1])), (int(bb2[2]), int(bb2[3])), colors['fp'],
                                  2)

                gt_file = tmp_files_path + "/" + pred + "_ground_truth.json"
                ground_truth_data = json.load(open(gt_file))
                list_fn_dict = [f1 for f1 in ground_truth_data if
                                f1["bbox"] not in [f["bbox"] for f in bb_matched if f['file_id'] == pred]]
                for bb1 in [tp2['bbox'] for tp2 in list_fn_dict]:
                    bb2 = [float(x) for x in bb1.split()]
                    cv2.rectangle(img_performance, (int(bb2[0]), int(bb2[1])), (int(bb2[2]), int(bb2[3])), colors['fn'],
                                  2)

                #cv2.imwrite(join(path_image_performance, pred + ".jpg"), img_performance)
                cv2.imshow("prova",img_performance)
                img_performance=cv2.resize(img_performance,(600,600))
                cv2.waitKey()

            total_tp[gt_classes.index(class_name)] += sum(tp)
            total_fp[gt_classes.index(class_name)] += sum(fp)
            total_gt[gt_classes.index(class_name)] += gt_counter_per_class[class_name]
            # print(tp)
            ...
            arr_prec = np.array(mprec)
            arr_rec = np.array(mrec)
            fscore = (2 * arr_prec * arr_rec) / (arr_prec + arr_rec)
            index_best_fscore = np.argmax(fscore)
            print("index_best_fscore:" + index_best_fscore.__str__())
            best_fscore = fscore[index_best_fscore]
            thresholds_list = sorted([f['confidence'] for f in predictions_data], reverse=True)
            # best_threshold = thresholds_list[index_best_fscore]
            # print("best_threshold:" + best_threshold.__str__())
            sub_performance_dict = {}
            sub_performance_dict.update({"AP": ap})
            sub_performance_dict.update({"Class": class_name})
            # sub_performance_dict.update({"best_threshold": best_threshold})
            sub_performance_dict.update({"mprec": mprec})
            sub_performance_dict.update({"mrec": mrec})
            sub_performance_dict.update({"rec": rec})
            sub_performance_dict.update({"prec": prec})
            sub_performance_dict.update({"indexFScore": index_best_fscore})
            sub_performance_dict.update({'Fscore': fscore})
            sub_performance_dict.update({"ThresholdList": thresholds_list})
            sub_performance_dict.update({"tp": total_tp[gt_classes.index(class_name)]})
            sub_performance_dict.update({"fp": total_fp[gt_classes.index(class_name)]})
            sub_performance_dict.update(
                {"fn": (gt_counter_per_class[class_name] - total_tp[gt_classes.index(class_name)]).__str__()})
            performance_dict.update({class_name: sub_performance_dict})

            from matplotlib import pyplot

            pyplot.plot(mrec, mprec, marker='.', label=args.output)
            # axis labels
      ...
      """
        ...
        print(text)
        performance_dict.update({"mAP": mAP})
        performance_dict.update({"sum_AP": sum_AP})
        performance_dict.update({"n_classes": n_classes})
        performance_dict.update({"IntersectionMethod": cfg.TEST.IntersectionMethod})
        performance_dict.update({"DIOU_NMS_THRESHOLD": cfg.TEST.DIOU_NMS_THRESHOLD})
        # performance_dict.update({"MaxCentroideDistance": cfg.TEST.MaxCentroideDistance})

        with open(join(args.output, cfg.TEST.performancefilename + ".pickle"), 'wb') as handle:
